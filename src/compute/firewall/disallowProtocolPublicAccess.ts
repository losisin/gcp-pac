...
export const disallowProtocolPublicAccess = {
	...
	description: "Check that Compute Firewall doesn't allow unrestricted access to protocol.",
		...
		if (args.type === 'gcp:compute/firewall:Firewall') {
			...
			if (sourceRanges && sourceRanges.includes('0.0.0.0/0')) {
				for (const rule of allows) {
					...
					if (!ports?.length) {
						reportViolation(
							`Compute Firewall rule should not allow unrestricted access to all ports for protocol ${protocol}.`
						)
					}
				}
			}
